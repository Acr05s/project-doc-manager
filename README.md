...
- **承建单位管理**：管理承建单位列表，支持指定项目经理为单位管理员，新建单位需PMO审批，支持批量删除
...
### v3.2.1
- **日报归档统计修复**：修复归档文档计数逻辑，归档数不再受限于同期上传数，归档率分母改为全量已上传文档数
- **通知类型选择**：新建/编辑定时任务增加"邮件"和"站内信"通知方式复选框，可单独控制是否发送
- **文档列表状态即时刷新**：归档、撤销归档、审批等操作后自动重新加载项目数据，文档状态即时更新
- **执行记录详情查看**：定时任务发送记录支持点击查看详情，包含邮件收件人地址、站内信收件人列表、发送状态
- **数据库迁移**：scheduled_tasks 表新增 email_enabled 字段

### v3.2.0
- **Dashboard 看板布局优化**：验收情况和资料完整两个统计图表并排横向显示，响应式布局
- **文档变化统计接入真实数据**：替换原有随机假数据为数据库真实记录，支持按日/3天/7天/30天查看
- **定时任务增强**：跳过周末/节假日选项对所有周期频率生效，新增"下一次执行时间"列和"跳过下次"按钮
- **乙方单位选择改进**：新建/编辑项目时乙方单位改为下拉选择，支持新建承建单位（需PMO审批）
- **刷新问题修复**：项目编辑保存后侧栏列表即时刷新，增加 Cache-Control 防止浏览器缓存
- **数据库迁移**：organizations 表新增 status 字段（承建单位审批状态）

### v3.1.0
- **定时任务调度器 _tick() 关键 bug 修复**：修复 _tick() 中 continue 后代码永远不可达的缩进错误
- **自动化增量迁移框架**：新增 tools/migrate/ 增量迁移框架，支持自动发现并执行编号迁移脚本
- **定时日报跳过非工作日功能**：新增中国法定节假日日历服务，日报类型任务支持"跳过非工作日"选项
- **定时任务管理弹窗 UI 优化**：Modal 窗口仅支持纵向拖拽调整大小，新增项目复选框筛选和周期筛选
- **编辑器弹窗增强**：频率选择"每日"时显示"跳过非工作日"复选框
- **数据库迁移**：scheduled_tasks 表新增 skip_holidays 字段
